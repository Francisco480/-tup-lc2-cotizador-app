...
function configurarEventosEliminar() {
    ...
    botonesEliminar.forEach(boton => {
        boton.addEventListener('click', function() {
            ...
            const moneda = fila.querySelector('.columna-moneda').textContent.trim();
            eliminarCotizacion(moneda);
        });
    });
}


// Función para eliminar una cotización por moneda
function eliminarCotizacion(nombreMoneda) {
    // Filtrar las cotizaciones para excluir la que se quiere eliminar
    cotizaciones = cotizaciones.filter(cotizacion => cotizacion.nombre !== nombreMoneda);
