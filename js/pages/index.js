    ...
    fetch("https://dolarapi.com/v1/dolares")
        ...
        .then(data => {
            ...
            if (mostrarTodos) {
                ...
                handleChange(); // Mostrar la cotización individual seleccionada
            }
...
// Función para mostrar cotizaciones según la selección del combobox
function handleChange() {
    ...
    if (seleccion === "Seleccionar todos") {
        ...
        mostrarTodos = false;
        const cotizacion = arregloIndex.find(item => item.nombre === seleccion);
        if (cotizacion) {
...
function mostrarCotizaciones() {
    ...
    arregloIndex.forEach(item => {
        ...
        `;
        plantillaPrecios.innerHTML += itemHtml; // Agregar nuevo elemento
    });
}
...
function mostrarCotizacionIndividual(cotizacion) {
    ...
    `;
    plantillaPrecios.innerHTML = itemHtml; // Mostrar único elemento
}
...
function actualizarCada5Minutos() {
    setInterval(() => {
        cargarCotizaciones();
    }, 60000); // 5 minutos en milisegundos  //ACOMODAR ESTO
}
